...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if TurkeyTrotPage.objects.filter(slug='turkey-trot-2025').exists():
            self.stdout.write(
                self.style.WARNING('Turkey Trot page already exists!')
            )
            return
        ...
        self.stdout.write(
            self.style.SUCCESS('Successfully created Turkey Trot page!')
        )
        self.stdout.write(
            self.style.SUCCESS('You can now access the Wagtail admin at /admin/')
        )